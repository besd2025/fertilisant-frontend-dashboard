    ...
    <Card className="w-full lg:w-[300px] lg:max-w-sm p-6 space-y-4 rounded-xl shadow-sm">
      <div className="flex flex-col items-center space-y-3 text-center">
        ...
        <div className="space-y-1">
          <p className="text-xl font-semibold">{data?.hangar_nom}</p>
        ...
        >
          <QrCode size={30} />
          <span className="">{data?.hangar_code}</span>
        </Badge>
      </div>
      ...
      <div className="space-y-3">
        <div>
          <div className="flex items-center justify-between text-sm flex-col gap-y-1">
            ...
            <span className="font-medium text-right">
              {
                data?.hangar_adress?.zone_code?.commune_code?.province_code
                  ?.province_name
              }{" "}
              / {data?.hangar_adress?.zone_code?.commune_code?.commune_name}
            </span>
          </div>
        ...
        <div>
          <div className="flex items-center justify-between text-sm flex-col gap-y-1">
            ...
            <span className="font-medium text-right">
              {data?.hangar_responsable?.user?.first_name}{" "}
              {data?.hangar_responsable?.user?.last_name}
            </span>
          </div>
          <Separator className="my-2" />
        </div>
        <div>
          <div className="flex items-center justify-between text-sm ">
            ...
            <span className="font-medium">
              {data?.hangar_responsable?.user?.phone}
            </span>
          </div>
          <Separator className="my-2" />
        </div>
