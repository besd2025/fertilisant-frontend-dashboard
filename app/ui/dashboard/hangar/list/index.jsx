...
export default function HangarsListTable() {
    ...
    const getHangars = async () => {
      try {
        ...
        const hangarData = results.map((hangar) => ({
          ...
          localite: {
            province:
              hangar?.hangar_adress?.zone_code?.commune_code?.province_code
                ?.province_name || "",
            commune:
              hangar?.hangar_adress?.zone_code?.commune_code?.commune_name ||
              "",
          },
        }));
  ...
  const columns = [
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start">
              ...
              <DropdownMenuItem
                onClick={() =>
                  navigator.clipboard.writeText(hangar.hangar.hangar_code)
                }
              >
                Copier code
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <Link
                href={`/fertilisant-dashboard/hangar/details/?id=${hangar?.id}`}
              >
    ...
    },
    {
      ...
      filterFn: (row, columnId, filterValue) => {
        ...
        return (
          hangar.hangar_name.toLowerCase().includes(search) ||
          hangar.hangar_code.toLowerCase().includes(search)
        );
      },
      cell: ({ row }) => {
