...
export default function Edit({ id }) {
  ...
  const [hangarName, setHangarName] = React.useState("");
  ...
  React.useEffect(() => {
    const getHangars = async () => {
      try {
        console.log("Fetching HANGAR data for ID:", id);
        const response = await fetchData("get", `cafe/stationslavage/${id}/`, {
          params: {},
          additionalHeaders: {},
          body: {},
        });
        console.log("HANGAR data fetched:", response);
        setCode(response?.hangar_code || "");
        setHangarName(response?.hangar_nom || "");
        setSoc(response?.societe?.nom_societe || "");
        setSocietteCode(response?.societe?.code_societe || "");
        setFirstName(response?.hangar_responsable?.user?.first_name || "");
        setLastName(response?.hangar_responsable?.user?.last_name || "");
        setTelephone(response?.hangar_responsable?.user?.phone || "");
        setProvince(
          response?.hangar_adress?.zone_code?.commune_code?.province_code
            ?.province_name || ""
        );
        setCommune(
          response?.hangar_adress?.zone_code?.commune_code?.commune_name || ""
        );
      } catch (error) {
